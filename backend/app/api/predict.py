...
from app.ml.model import model_registry, FEATURE_COLS
...
def predict(request: PredictRequest, db: Session = Depends(get_db)):
    # Build overrides from all optional fields the caller may have provided
    overrides = {
        col: getattr(request, col, None)
        for col in FEATURE_COLS
    }

    # Sector defaults fill anything not overridden
